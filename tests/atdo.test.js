...
/**
 * atdo skill 自动化测试
 * 用法: bash scripts/test-unit.sh (推荐入口) / node tests/atdo.test.js (直接调)
 * 退出码: 0 = 全部通过, 1 = 有失败
 *
 * 测试内容(按命令):
...
// ─────────────────────────────────────────────────────────────
// set-project-dirs / get-project-dirs 命令
// ─────────────────────────────────────────────────────────────
// 命令: set-project-dirs
//   stdin: JSON { processDir: "_scratch/", testDir: "tests/" }
//   将项目临时目录约定写入 state.json.projectDirs
//   无 state.json 时创建最小 state（Step 3.5 可能在 init 前调用）
// 命令: get-project-dirs [key]
//   查询已配置的项目目录

describe('set-project-dirs / get-project-dirs', () => {
  let dir;
  before(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-pdirs-'));
  });
  after(() => { fs.rmSync(dir, { recursive: true, force: true }); });

  // 1
  test('正常设置 processDir + testDir', () => {
    const r = runWithStdin(dir, '{"processDir":"_scratch/","testDir":"specs/"}', 'set-project-dirs');
    assert.equal(r.code, 0, r.stderr);
    const out = JSON.parse(r.stdout);
    assert.ok(out.ok);
    assert.equal(out.projectDirs.processDir, '_scratch/');
    assert.equal(out.projectDirs.testDir, 'specs/');
  });

  // 2
  test('无 state.json 时创建最小 state', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-pdirs2-'));
    try {
      const r = runWithStdin(d, '{"processDir":"_scratch/"}', 'set-project-dirs');
      assert.equal(r.code, 0, r.stderr);
      ...
      const state = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
      assert.equal(state.projectDirs.processDir, '_scratch/');
    } finally {
  ...
  // 3
  test('init 后 projectDirs 被保留', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-pdirs3-'));
    try {
      // 先 set-project-dirs
      let r = runWithStdin(d, '{"processDir":"_scratch/"}', 'set-project-dirs');
      assert.equal(r.code, 0, r.stderr);
      // 再 init
      r = runWithStdin(d, '{"phases":[{"name":"t","tasks":["t1"]}]}', 'init');
      assert.equal(r.code, 0, r.stderr);
      // projectDirs 应被保留
      const stateFile = path.join(d, '.phase-execution', 'state.json');
      const state = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
      assert.equal(state.projectDirs.processDir, '_scratch/');
      assert.ok(state.phases, 'init 的 phases 也应存在');
    } finally {
      fs.rmSync(d, { recursive: true, force: true });
    }
  });
  ...
  // 4
  test('processDir 不以 / 结尾 → die', () => {
    const r = runWithStdin(dir, '{"processDir":"scratch"}', 'set-project-dirs');
    assert.notEqual(r.code, 0);
    assert.match(r.stderr, /必须以.*\/.*结尾/);
  });

  // 5
  test('processDir 为空字符串 → die', () => {
    const r = runWithStdin(dir, '{"processDir":""}', 'set-project-dirs');
    assert.notEqual(r.code, 0);
    assert.match(r.stderr, /必须是非空字符串/);
  });

  // 6
  test('无效 JSON → die', () => {
    const r = runWithStdin(dir, 'not json', 'set-project-dirs');
    assert.notEqual(r.code, 0);
    assert.match(r.stderr, /需要有效 JSON/);
  });

  // 7
  test('processDir 非字符串 → die', () => {
    const r = runWithStdin(dir, '{"processDir":123}', 'set-project-dirs');
    assert.notEqual(r.code, 0);
    assert.match(r.stderr, /必须是非空字符串/);
  });

  // 8
  test('get-project-dirs 返回完整 JSON', () => {
    const r = runWithStdin(dir, '{"processDir":"_scratch/","testDir":"specs/"}', 'set-project-dirs');
    assert.equal(r.code, 0);
    const r2 = spawnSync('node', [SCRIPT, 'get-project-dirs'], { encoding: 'utf8', cwd: dir, env: { ...process.env, FORCE_COLOR: '0' } });
    const out = JSON.parse(r2.stdout.trim());
    assert.equal(out.processDir, '_scratch/');
    assert.equal(out.testDir, 'specs/');
  });

  // 9
  test('get-project-dirs processDir 返回单个值', () => {
    const r2 = spawnSync('node', [SCRIPT, 'get-project-dirs', 'processDir'], { encoding: 'utf8', cwd: dir, env: { ...process.env, FORCE_COLOR: '0' } });
    assert.equal(r2.stdout.trim(), '_scratch/');
  });

  // 10
  test('get-project-dirs 无 state.json 返回 null', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-pdirs4-'));
    try {
      const r2 = spawnSync('node', [SCRIPT, 'get-project-dirs'], { encoding: 'utf8', cwd: d, env: { ...process.env, FORCE_COLOR: '0' } });
      assert.equal(r2.stdout.trim(), 'null');
    } finally {
      fs.rmSync(d, { recursive: true, force: true });
    }
  });
...
describe('--tdd: TDD 模式注入', () => {
  test('init 接受 plan.options.tdd: true → state.tddMode: true', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-tdd-1-'));
    try {
      const plan = JSON.stringify({
        phases: [{ name: 'Phase 01', tasks: ['task a'] }],
        options: { tdd: true },
      });
      const r = initPlan(d, plan);
      assert.equal(r.code, 0, r.stderr);
      const state = JSON.parse(fs.readFileSync(path.join(d, '.phase-execution/state.json'), 'utf8'));
      assert.equal(state.tddMode, true, 'plan.options.tdd: true 应写入 state.tddMode');
      const out = JSON.parse(r.stdout);
      assert.ok(out.ok);
      assert.equal(out.phases, 1);
    } finally { fs.rmSync(d, { recursive: true, force: true }); }
  });

  test('plan.options.tdd 缺失 → state.tddMode 不存在(向后兼容)', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-tdd-2-'));
    try {
      const plan = JSON.stringify({
        phases: [{ name: 'Phase 01', tasks: ['task a'] }],
      });
      ...
      assert.equal(r.code, 0, r.stderr);
      const state = JSON.parse(fs.readFileSync(path.join(d, '.phase-execution/state.json'), 'utf8'));
      assert.ok(!('tddMode' in state), '无 options.tdd → state 不应有 tddMode 字段');
    } finally { fs.rmSync(d, { recursive: true, force: true }); }
  });

  test('plan.options.tdd 不是 boolean → FATAL', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-tdd-3-'));
    try {
      const plan = JSON.stringify({
        phases: [{ name: 'Phase 01', tasks: ['task a'] }],
        options: { tdd: 'yes' },
      ...
      const r = initPlan(d, plan);
      assert.notEqual(r.code, 0, 'options.tdd 不是 boolean 应 FATAL');
      assert.match(r.stderr, /tdd/);
    } finally { fs.rmSync(d, { recursive: true, force: true }); }
  });

  test('get tddMode → 返回 true', () => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-tdd-4-'));
    try {
      initPlan(d, JSON.stringify({ phases: [{ name: 'a', tasks: ['t1'] }], options: { tdd: true } }));
      const r = runIn(d, 'get', 'tddMode');
      assert.equal(r.code, 0, r.stderr);
      assert.equal(r.stdout, 'true');
    } finally { fs.rmSync(d, { recursive: true, force: true }); }
  });

  test('SKILL.md 包含 --tdd Argument 描述', () => {
    const skillContent = fs.readFileSync(path.join(__dirname, '../SKILL.md'), 'utf8');
    assert.match(skillContent, /--tdd/);
  });

  test('SKILL.md 包含 TDD 协议注入块', () => {
    const skillContent = fs.readFileSync(path.join(__dirname, '../SKILL.md'), 'utf8');
    assert.match(skillContent, /TDD 协议/);
    assert.match(skillContent, /RED.*GREEN.*REFACTOR/);
  });
});

// ─── atdo-01: agent-spawn 报告 SUCCESS 但未生成 audit-report.md ───
// Bug: Step 4 spawn gsd-code-reviewer agent, agent 口头报 SUCCESS 但 audit-report.md
//      未生成。根因: orchestrator 未声明 subagent_type(可能 fallback 到内置只读
//      code-reviewer),且 spawn 后未验证产物存在。
// 修复: SKILL.md Step 4 / Step 7 显式声明 subagent_type + spawn 后立即 test -f
//      验证 + 缺失触发 fix loop。
describe('atdo-01: agent-spawn 产物验证协议', () => {
  ...
  test('Step 4: 显式声明 subagent_type: gsd-code-reviewer', () => {
    // 抽取 Step 4 spawn prompt 段
    const step4Section = extractSection(skillContent, '**4. Agent Audit**', '**5. Fix Loop**');
    assert.match(
      step4Section,
      /subagent_type:\s*["']?gsd-code-reviewer["']?/,
      'Step 4 spawn prompt 必须显式声明 subagent_type,避免 fallback 到内置只读 code-reviewer',
    );
  });

  test('Step 4: spawn 后立即 test -f 验证 audit-report.md 存在', () => {
    const step4Section = extractSection(skillContent, '**4. Agent Audit**', '**5. Fix Loop**');
    // 接受三种形式:test -f / [ -f / [ ! -f
    assert.match(
      step4Section,
      /(test\s+-f|\[\s+!{0,1}\s*-f)/,
      'Step 4 spawn 后必须立即验证 audit-report.md 存在(test -f / [ -f / [ ! -f)',
    );
    assert.match(
      step4Section,
      /\.phase-execution\/phases\/\$\{?phaseId\}?\/audit-report\.md/,
      'Step 4 必须显式提及 audit-report.md 路径(在变量定义或字面路径中)',
    );
    assert.match(
      step4Section,
      /(test\s+-s|\[\s+!{0,1}\s*-s)/,
      'Step 4 验证必须包含 -s(文件非空)检查,空文件也视为产物缺失',
    );
  });

  test('Step 4: 产物缺失时明确触发 fix loop 而非静默跳过', () => {
    const step4Section = extractSection(skillContent, '**4. Agent Audit**', '**5. Fix Loop**');
    // 验证产物缺失 → fix loop 关键词共现
    assert.match(
      step4Section,
      /audit-report\.md[\s\S]{0,500}(缺失|不存在|missing|fallback)/i,
      '产物缺失路径必须显式提及(不只是 sanitize die 静默放过)',
    );
    assert.match(
      step4Section,
      /(缺失|不存在|missing)[\s\S]{0,500}(fix loop|inc-strike|3-strike|ALERT)/i,
      '产物缺失时必须连接到 fix loop 或 ALERT 协议',
    );
  });

  test('Step 4: 使用 inc-strike fix 维度(复用现有 fix-loop 协议)', () => {
    const step4Section = extractSection(skillContent, '**4. Agent Audit**', '**5. Fix Loop**');
    assert.match(
      step4Section,
      /inc-strike\s+<\??phaseId\??>\s+fix/,
      '产物缺失时复用 fix 维度 strike(沿用现有 3-strike 协议,无需新增 type)',
    );
  });

  test('Step 7: 显式声明 subagent_type: gsd-integration-checker', () => {
    const step7Section = extractSection(skillContent, '**7. Gate Integration Test**', '**8. Git Commit');
    assert.match(
      step7Section,
      /subagent_type:\s*["']?gsd-integration-checker["']?/,
      'Step 7 spawn prompt 必须显式声明 subagent_type(同 Step 4 同病)',
    );
  });

  test('Step 7: spawn 后立即 test -f 验证 integration-test-report.md 存在', () => {
    const step7Section = extractSection(skillContent, '**7. Gate Integration Test**', '**8. Git Commit');
    assert.match(
      step7Section,
      /(test\s+-f|\[\s+!{0,1}\s*-f)/,
      'Step 7 spawn 后必须立即验证 integration-test-report.md 存在',
    );
    assert.match(
      step7Section,
      /\.phase-execution\/gates\/[^/]+\/integration-test-report\.md/,
      'Step 7 必须显式提及 integration-test-report.md 路径',
    );
    assert.match(
      step7Section,
      /(test\s+-s|\[\s+!{0,1}\s*-s)/,
      'Step 7 验证必须包含 -s(文件非空)检查',
    );
  });

  test('Step 7: integration 报告缺失触发 fix loop', () => {
    const step7Section = extractSection(skillContent, '**7. Gate Integration Test**', '**8. Git Commit');
    assert.match(
      step7Section,
      /integration-test-report\.md[\s\S]{0,500}(缺失|不存在|missing)/i,
      'Step 7 报告缺失路径必须显式提及',
    );
    assert.match(
      step7Section,
      /(缺失|不存在|missing)[\s\S]{0,500}(inc-strike|ALERT|3-strike)/i,
      'Step 7 报告缺失时连接到 inc-strike 或 ALERT',
    );
  });
});

// ─── P0-1: Task Timeout & Heartbeat Protocol ───────────
// 任务:实现任务级加权超时 + 5/10/15 min 三级心跳响应
// 协议:见 SKILL.md "Task Timeout & Heartbeat Protocol (P0-1)"
describe('P0-1: Task Timeout & Heartbeat Protocol', () => {
  const phaseStatePath = path.join(__dirname, '..', 'scripts', 'phase-state.js');
  const watchdogPath = path.join(__dirname, '..', 'scripts', 'watchdog.sh');

  describe('compute-timeout 加权计算(5 complexity × 3 attempt)', () => {
    function callComputeTimeout(...args) {
      const r = spawnSync('node', [phaseStatePath, 'compute-timeout', ...args], { encoding: 'utf8' });
      if (r.status !== 0) {
        throw new Error(`compute-timeout 退出码 ${r.status}: ${r.stderr}`);
      }
      return JSON.parse(r.stdout);
    }

    test('trivial attempt 1 → 5 min(base)', () => {
      assert.equal(callComputeTimeout('trivial', '1').minutes, 5);
    });
    test('small attempt 1 → 8 min(base)', () => {
      assert.equal(callComputeTimeout('small', '1').minutes, 8);
    });
    test('medium attempt 1 → 12 min(base,默认值)', () => {
      assert.equal(callComputeTimeout('medium', '1').minutes, 12);
    });
    test('large attempt 1 → 20 min(base)', () => {
      assert.equal(callComputeTimeout('large', '1').minutes, 20);
    });
    test('huge attempt 1 → 30 min(base)', () => {
      assert.equal(callComputeTimeout('huge', '1').minutes, 30);
    });
    test('medium attempt 2 → 14 min(12 × 1.15 = 13.8 → round 14)', () => {
      assert.equal(callComputeTimeout('medium', '2').minutes, 14);
    });
    test('medium attempt 3 → 16 min(12 × 1.30 = 15.6 → round 16)', () => {
      assert.equal(callComputeTimeout('medium', '3').minutes, 16);
    });
    test('large attempt 3 → 26 min(20 × 1.30 = 26)', () => {
      assert.equal(callComputeTimeout('large', '3').minutes, 26);
    });
    test('越界上限:huge attempt 10 → clamp to 35', () => {
      assert.equal(callComputeTimeout('huge', '10').minutes, 35);
    });
    test('未知 complexity → fallback 12(同 medium)', () => {
      assert.equal(callComputeTimeout('unknown-complexity', '1').minutes, 12);
    });
    test('无参数 → 默认 medium attempt 1 → 12', () => {
      assert.equal(callComputeTimeout().minutes, 12);
    });
    test('attempt 必须 ≥ 1,非法值 die', () => {
      const r = spawnSync('node', [phaseStatePath, 'compute-timeout', 'medium', '0'], { encoding: 'utf8' });
      assert.equal(r.status, 1);
      assert.match(r.stderr, /attempt 必须/);
    });
    test('返回 isoDeadline 字段 + formula 字段', () => {
      const r = callComputeTimeout('large', '2');
      assert.ok(r.isoDeadline, '应返回 isoDeadline');
      assert.ok(r.formula, '应返回 formula');
      assert.match(r.formula, /min\(35, max\(3/);
    });
  });

  describe('set-task-deadline 写入验证', () => {
    function setupTmp() {
      const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p0-1-td-'));
      fs.mkdirSync(path.join(d, '.phase-execution'), { recursive: true });
      // 写一个 state.json 包含 currentPhaseIndex 指向 phase 01
      fs.writeFileSync(path.join(d, '.phase-execution/state.json'), JSON.stringify({
        currentPhaseIndex: 0,
        phases: [{ number: '01', name: 'test', status: 'pending', tasks: [] }],
      }));
      return d;
    }

    test('正常 ISO 时间戳 → 写入 state.json.taskTimeouts + ok: true', () => {
      const d = setupTmp();
      try {
        const r = spawnSync('node', [phaseStatePath, 'set-task-deadline', '01', '2026-12-31T23:59:59Z'], {
          encoding: 'utf8', cwd: d,
        });
        assert.equal(r.status, 0, `stderr=${r.stderr}`);
        const out = JSON.parse(r.stdout);
        assert.equal(out.ok, true);
        assert.equal(out.phaseId, '01');
        // state.json.taskTimeouts[01].deadline 已写入
        const state = JSON.parse(fs.readFileSync(path.join(d, '.phase-execution/state.json'), 'utf8'));
        assert.equal(state.taskTimeouts['01'].deadline, '2026-12-31T23:59:59Z');
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });
    ...
    test('无效 ISO 时间戳 → die', () => {
      const d = setupTmp();
      try {
        const r = spawnSync('node', [phaseStatePath, 'set-task-deadline', '01', 'not-a-date'], {
          encoding: 'utf8', cwd: d,
        });
        assert.equal(r.status, 1);
        assert.match(r.stderr, /无效 ISO/);
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('无 phaseId → die', () => {
      const r = spawnSync('node', [phaseStatePath, 'set-task-deadline'], { encoding: 'utf8' });
      assert.equal(r.status, 1);
      assert.match(r.stderr, /需要 phaseId/);
    });

    test('无 isoTime → die', () => {
      const r = spawnSync('node', [phaseStatePath, 'set-task-deadline', '01'], { encoding: 'utf8' });
      assert.equal(r.status, 1);
      assert.match(r.stderr, /需要 ISO 时间戳/);
    });
  });

  describe('writeHeartbeat taskDeadline 扩展', () => {
    function setupTmp() {
      const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p0-1-hb-'));
      fs.mkdirSync(path.join(d, '.phase-execution'), { recursive: true });
      return d;
    }

    test('不带 taskDeadline → heartbeat.json 不含 taskDeadline 字段', () => {
      const d = setupTmp();
      try {
        spawnSync('node', [phaseStatePath, 'heartbeat', '01', '', 'active'], { encoding: 'utf8', cwd: d });
        const hb = JSON.parse(fs.readFileSync(path.join(d, '.phase-execution/heartbeat.json'), 'utf8'));
        assert.equal(hb.currentPhase, '01');
        assert.equal(hb.status, 'active');
        assert.equal(hb.taskDeadline, undefined, '无 taskDeadline 参数时,字段不应出现');
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('带 taskDeadline → heartbeat.json 含 taskDeadline 字段', () => {
      const d = setupTmp();
      try {
        const r = spawnSync('node', [phaseStatePath, 'heartbeat', '01', '', 'active', '2026-12-31T23:59:59Z'], {
          encoding: 'utf8', cwd: d,
        });
        assert.equal(r.status, 0, `stderr=${r.stderr}`);
        const hb = JSON.parse(fs.readFileSync(path.join(d, '.phase-execution/heartbeat.json'), 'utf8'));
        assert.equal(hb.taskDeadline, '2026-12-31T23:59:59Z');
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('无效 taskDeadline → die', () => {
      const d = setupTmp();
      try {
        const r = spawnSync('node', [phaseStatePath, 'heartbeat', '01', '', 'active', 'bad-date'], {
          encoding: 'utf8', cwd: d,
        });
        assert.equal(r.status, 1);
        assert.match(r.stderr, /taskDeadline 无效/);
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });
  });

  describe('P1-1: 默认 audit 关闭', () => {
    // 协议(SKILL.md §4 Agent Audit):audit 默认 off;仅 --audit 才 spawn gsd-code-reviewer
    // 命令: node scripts/phase-state.js get-audit-default → 输出 'off' 或 'on'
    // 实现:从 state.json 顶层 noAudit 字段推断
    //   - noAudit === false → 'on'(用户显式 --audit)
    //   - 其他(noAudit 未设 / noAudit === true) → 'off'(默认或显式 --no-audit)

    const phaseStatePath = path.join(__dirname, '..', 'scripts', 'phase-state.js');

    function setupTmp() {
      const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p1-1-'));
      fs.mkdirSync(path.join(d, '.phase-execution'), { recursive: true });
      return d;
    }

    test('默认(state.noAudit 未设)→ 输出 off', () => {
      const d = setupTmp();
      try {
        const r = spawnSync('node', [phaseStatePath, 'get-audit-default'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 0, `stderr=${r.stderr}`);
        assert.equal(r.stdout, 'off');
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('state.noAudit === true → 输出 off', () => {
      const d = setupTmp();
      try {
        fs.writeFileSync(path.join(d, '.phase-execution/state.json'),
          JSON.stringify({ noAudit: true, phases: [] }));
        const r = spawnSync('node', [phaseStatePath, 'get-audit-default'], { encoding: 'utf8', cwd: d });
    ...
    test('state.noAudit === false → 输出 on', () => {
      const d = setupTmp();
      try {
        fs.writeFileSync(path.join(d, '.phase-execution/state.json'),
          JSON.stringify({ noAudit: false, phases: [] }));
        const r = spawnSync('node', [phaseStatePath, 'get-audit-default'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 0);
        assert.equal(r.stdout, 'on');
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('state.json 损坏 → 输出 off(graceful fallback)', () => {
      const d = setupTmp();
      try {
        fs.writeFileSync(path.join(d, '.phase-execution/state.json'), '{not valid json');
        const r = spawnSync('node', [phaseStatePath, 'get-audit-default'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 0);
        assert.equal(r.stdout, 'off');
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    ...
    test('SKILL.md argument-hint 含 [--audit]', () => {
      const skillContent = fs.readFileSync(
        path.join(__dirname, '..', 'SKILL.md'), 'utf8');
      assert.match(skillContent, /\[--audit\]/);
    });

    test('SKILL.md flag 表格说明默认 off 或 --no-audit 兼容', () => {
      const skillContent = fs.readFileSync(
        path.join(__dirname, '..', 'SKILL.md'), 'utf8');
      assert.match(skillContent, /--audit.*默认.*off|--no-audit.*no-op 兼容/);
    });
  });

  describe('P2-1: plan 格式自动转化(检测/备份/核对/转化)', () => {
    // 协议(SKILL.md §Plan Conversion Protocol):phase-state.js 提供机械原语
    //   detect-plan-tier / plan-validate / plan-backup / plan-diff / transform-tier2
    // 边界:stdin 模式不可备份;HTML 注释内 `-` 行不计入 task 数
    const phaseStatePath = path.join(__dirname, '..', 'scripts', 'phase-state.js');

    describe('detect-plan-tier 探测 tier + needsConvert 信号', () => {
      function setupTmp(content) {
        const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p2-1-detect-'));
        fs.writeFileSync(path.join(d, 'plan.md'), content, 'utf8');
        return d;
      }

      test('Tier 3.5 正常 → tier=task-list, valid=true, exit 0', () => {
        const d = setupTmp('# Plan\n\n### P0-1: 测试\n- task A\n- task B\n');
        try {
          const r = spawnSync('node', [phaseStatePath, 'detect-plan-tier', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 0);
          const out = JSON.parse(r.stdout);
          assert.equal(out.tier, 'task-list');
          assert.equal(out.valid, true);
          assert.equal(out.needsConvert, false);
        } finally { fs.rmSync(d, { recursive: true, force: true }); }
      });

      test('JSON 正常 → tier=phase-sequence, valid=true', () => {
        const d = setupTmp('{"phases":[{"name":"a","tasks":["x"]}]}');
        try {
          const r = spawnSync('node', [phaseStatePath, 'detect-plan-tier', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 0);
          const out = JSON.parse(r.stdout);
          assert.equal(out.tier, 'phase-sequence');
          assert.equal(out.valid, true);
        } finally { fs.rmSync(d, { recursive: true, force: true }); }
      });

      test('P4-N 优先级越界 → needsConvert=true, exit 2', () => {
        const d = setupTmp('### P4-1: 越界\n- task\n');
        try {
          const r = spawnSync('node', [phaseStatePath, 'detect-plan-tier', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 2);
          const out = JSON.parse(r.stdout);
          assert.equal(out.needsConvert, true);
          assert.ok(out.issues.some(i => /priority/.test(i)));
        } finally { fs.rmSync(d, { recursive: true, force: true }); }
      });

      test('完全无效 → needsConvert=true, issues 列原因', () => {
        const d = setupTmp('完全不是 plan 的随机文本');
        try {
          const r = spawnSync('node', [phaseStatePath, 'detect-plan-tier', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 2);
    ...
    describe('plan-validate 校验 + issues 列表', () => {
      test('正常 Tier 3.5 → valid=true', () => {
        const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p2-1-val-'));
        try {
          fs.writeFileSync(path.join(d, 'plan.md'), '### P0-1: t\n- task A\n');
          const r = spawnSync('node', [phaseStatePath, 'plan-validate', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 0);
          const out = JSON.parse(r.stdout);
          assert.equal(out.valid, true);
      ...
      test('JSON 缺 phases → valid=false + issue 提示', () => {
        const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p2-1-val-'));
        try {
          fs.writeFileSync(path.join(d, 'plan.md'), '{"foo":"bar"}');
          const r = spawnSync('node', [phaseStatePath, 'plan-validate', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 0);
          const out = JSON.parse(r.stdout);
          assert.equal(out.valid, false);
          assert.ok(out.issues.some(i => /phases/.test(i)));
        } finally { fs.rmSync(d, { recursive: true, force: true }); }
      });
    });

    describe('plan-backup 备份机制', () => {
      test('备份创建 <file>.orig.<ISO-ts>.md', () => {
        const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p2-1-backup-'));
        try {
          fs.writeFileSync(path.join(d, 'plan.md'), 'content');
          const r = spawnSync('node', [phaseStatePath, 'plan-backup', 'plan.md'],
            { encoding: 'utf8', cwd: d });
          assert.equal(r.status, 0);
          const out = JSON.parse(r.stdout);
          assert.match(out.backupPath, /plan\.md\.orig\..+\.md$/);
          assert.ok(fs.existsSync(path.join(d, out.backupPath)), '备份文件应存在');
        } finally { fs.rmSync(d, { recursive: true, force: true }); }
      });

      test('stdin 模式 → die(不支持)', () => {
        const r = spawnSync('node', [phaseStatePath, 'plan-backup', '-'], { encoding: 'utf8' });
        assert.equal(r.status, 1);
    ...
    describe('plan-diff 任务数核对', () => {
      test('delta=0 → ok', () => {
        const orig = JSON.stringify({ rawText: '# Plan\n- a\n- b\n' });
        const converted = JSON.stringify({ phases: [{ tasks: ['a', 'b'] }] });
        const r = spawnSync('node', [phaseStatePath, 'plan-diff', orig, converted],
          { encoding: 'utf8' });
        assert.equal(r.status, 0);
        const out = JSON.parse(r.stdout);
      ...
      test('delta≠0 → die(转化丢失任务)', () => {
        const orig = JSON.stringify({ rawText: '# Plan\n- a\n- b\n- c\n' });
        const converted = JSON.stringify({ phases: [{ tasks: ['a'] }] });
        const r = spawnSync('node', [phaseStatePath, 'plan-diff', orig, converted],
          { encoding: 'utf8' });
        assert.equal(r.status, 1);
        assert.match(r.stderr, /转化丢失任务/);
      });

      test('HTML 注释内 `-` 行不计入', () => {
        const orig = JSON.stringify({ rawText: '<!--\n- 注释 1\n- 注释 2\n-->\n\n# Plan\n- real task\n' });
        const converted = JSON.stringify({ phases: [{ tasks: ['real task'] }] });
        const r = spawnSync('node', [phaseStatePath, 'plan-diff', orig, converted],
          { encoding: 'utf8' });
        assert.equal(r.status, 0, 'HTML 注释内的 `-` 行不算 task');
      });
    });

    describe('transform-tier2 Tier 3.5 → Tier 2 JSON', () => {
      test('正常 Tier 3.5 → 输出标准 Tier 2 JSON + _meta', () => {
        const r = spawnSync('node', [phaseStatePath, 'transform-tier2',
          '# Plan\n\n### P0-1: t1\n- task A\n- task B\n\n### P1-1: t2\n- task C\n'],
          { encoding: 'utf8' });
        assert.equal(r.status, 0);
        const out = JSON.parse(r.stdout);
        assert.equal(out.phases.length, 2);
        assert.equal(out.phases[0].name, 'P0-1: t1');
        assert.equal(out.phases[0].tasks.length, 2);
        assert.equal(out.phases[1].number, '02');
        assert.equal(out._meta.convertedFrom, 'task-list');
      });

      test('非 Tier 3.5 格式 → die', () => {
        const r = spawnSync('node', [phaseStatePath, 'transform-tier2', '无 P?-N 标题'],
          { encoding: 'utf8' });
        assert.equal(r.status, 1);
        assert.match(r.stderr, /parseTaskListPlan 返回空/);
      });
    });

    describe('SKILL.md 协议层验证', () => {
      test('含 "Plan Conversion Protocol" 章节', () => {
        const skillContent = fs.readFileSync(
          path.join(__dirname, '..', 'SKILL.md'), 'utf8');
        assert.match(skillContent, /### Plan Conversion Protocol \(P2-1\)/);
      });

      test('含 "Step 2.5: Plan Format Auto-Convert" 协议段', () => {
        const skillContent = fs.readFileSync(
          path.join(__dirname, '..', 'SKILL.md'), 'utf8');
        assert.match(skillContent, /Step 2\.5: Plan Format Auto-Convert/);
      });
    });
  });

  describe('watchdog.sh 三级响应 + taskDeadline 集成', () => {
    function setupTmp() {
      const d = fs.mkdtempSync(path.join(os.tmpdir(), 'atdo-p0-1-wd-'));
      const sd = path.join(d, '.phase-execution');
      fs.mkdirSync(sd, { recursive: true });
      fs.writeFileSync(path.join(sd, 'state.json'), JSON.stringify({
        currentPhaseIndex: 0,
        phases: [{ number: '01', name: 'test', status: 'in_progress', tasks: [] }],
        strikes: { phaseRetry: {}, regression: 0, sameCategory: {} },
      }));
      return d;
    }

    test('心跳新鲜(< 5min)+ 无 taskDeadline → 心跳正常,exit 0', () => {
      const d = setupTmp();
      try {
        const hb = { timestamp: new Date().toISOString(), pid: process.pid, status: 'active' };
        fs.writeFileSync(path.join(d, '.phase-execution/heartbeat.json'), JSON.stringify(hb));
        const r = spawnSync('bash', [watchdogPath, 'check-heartbeat'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 0, `stderr=${r.stderr}`);
        assert.match(r.stdout, /心跳正常/);
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('diff=600s(10min 硬警告边界)+ 无 taskDeadline → exit 1 + strike 写入', () => {
      const d = setupTmp();
      try {
        // 构造 10 分钟前的时间戳
        const pastTs = new Date(Date.now() - 600 * 1000).toISOString();
        const hb = { timestamp: pastTs, pid: process.pid, status: 'active' };
        fs.writeFileSync(path.join(d, '.phase-execution/heartbeat.json'), JSON.stringify(hb));
        const r = spawnSync('bash', [watchdogPath, 'check-heartbeat'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 1, '硬警告应 exit 1');
        assert.match(r.stdout, /\[STRIKE\]/);
        // 验证 state.strikes 累加
        const state = JSON.parse(fs.readFileSync(path.join(d, '.phase-execution/state.json'), 'utf8'));
        assert.ok(state.strikes.sameCategory['slow-heartbeat'] >= 1,
          `slow-heartbeat strike 应 ≥ 1,实际: ${JSON.stringify(state.strikes)}`);
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('diff=900s(15min kill 边界)+ 无 taskDeadline → exit 1 + kill 行为', () => {
      const d = setupTmp();
      try {
        const pastTs = new Date(Date.now() - 900 * 1000).toISOString();
        const hb = { timestamp: pastTs, pid: process.pid, status: 'active' };
        fs.writeFileSync(path.join(d, '.phase-execution/heartbeat.json'), JSON.stringify(hb));
        const r = spawnSync('bash', [watchdogPath, 'check-heartbeat'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 1, 'kill 应 exit 1');
        assert.match(r.stdout, /\[KILL\]/);
      } finally { fs.rmSync(d, { recursive: true, force: true }); }
    });

    test('diff=300s(5min 软警告边界)+ 无 taskDeadline → exit 0 + WARN 输出', () => {
      const d = setupTmp();
      try {
        const pastTs = new Date(Date.now() - 300 * 1000).toISOString();
        const hb = { timestamp: pastTs, pid: process.pid, status: 'active' };
        fs.writeFileSync(path.join(d, '.phase-execution/heartbeat.json'), JSON.stringify(hb));
        const r = spawnSync('bash', [watchdogPath, 'check-heartbeat'], { encoding: 'utf8', cwd: d });
        assert.equal(r.status, 0, '软警告应 exit 0(不写 state)');
